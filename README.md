...
2. **没在运行就自动启动** —— 自动运行 `dsh web` 把服务拉起来，并等它就绪（不需要你手动敲命令）
3. **自动挑选浏览器** —— 优先用夸克浏览器；**没装夸克也没关系，会自动改用你电脑的默认浏览器**（Chrome / Edge / Firefox 等都可以）
4. **打开 DSH 界面** —— 浏览器自动弹出 DSH 页面
...
|---|---|
| Windows 10 或 11 | 必须 |
| DSH 已经安装（`dsh` 命令可用） | 本工具是"打开 DSH"，不是"安装 DSH"。如果 DSH 没在运行，工具会自动运行 `dsh web` 把它拉起来 |
...
- 若日志里有"找不到 dsh 命令"：说明 DSH 没装好（`dsh` 命令不可用），请先安装 DSH；若日志里有"等待超时"：说明自动启动失败，请手动在命令行运行 `dsh web` 排查
...
# 不自动启动 dsh web（默认会自动启动；如果你已经自己开了 DSH，可加这个参数跳过）
powershell -ExecutionPolicy Bypass -File open-dsh.ps1 -NoAutoStart
```
